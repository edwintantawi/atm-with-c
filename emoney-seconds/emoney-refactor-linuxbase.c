...
int activeUser = -1, numOfUsers = 2, transactionHistoryLength = 32, transactionLabels = 0;
...
int TransferController(struct Users user[numOfUsers], struct TransactionHistory transactionHistory[transactionHistoryLength], char inputTransferTo[10], int inputBalanceAmount);
...
int TransferController(struct Users users[numOfUsers], struct TransactionHistory transactionHistory[transactionHistoryLength], char inputTransferTo[10], int inputBalanceAmount){
  int indexOfTransferToUser = -1;
  int minTransferBalanceAmount = 100;
  for (int indexOfUser = 0; indexOfUser < numOfUsers; indexOfUser++) {
    int isValidIdCard = strcmp(users[indexOfUser].idCard, inputTransferTo);
    int isNotMine = strcasecmp(users[activeUser].idCard, inputTransferTo);
...
void TransactionView(struct Users users[numOfUsers], struct TransactionHistory transactionHistory[transactionHistoryLength]){
  ...
  do{
    ...
    {
      int isYouTransactionFrom = strcasecmp(transactionHistory[transactionLabel].from, users[activeUser].idCard);
      int isYouTransactionTo = strcasecmp(transactionHistory[transactionLabel].to, users[activeUser].idCard);
